    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            labels: [
                <?php
                $data = null;
                if (count($penjualan_makanan) > count($penjualan_minuman)) {
                    $data = $penjualan_makanan;
                } else if (count($penjualan_makanan) < count($penjualan_minuman)) {
                    $data = $penjualan_minuman;
                } else {
                    $data = $penjualan_makanan;
                }
            ...
            datasets: [{
                label: 'Total Penjualan Makanan',
                data: [<?php foreach ($penjualan_makanan as $dm) { ?> <?= $dm->makanan  ?>,
                    <?php } ?>
                ],
                backgroundColor: [
                    'rgba(60,141,188,1)'
                ...
                data: [<?php foreach ($penjualan_minuman as $dmn) { ?> <?= $dmn->minuman ?>,
                    <?php } ?>
                ],
                backgroundColor: [
                    'rgba(210, 214, 222, 1)'
